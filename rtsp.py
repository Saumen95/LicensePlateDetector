...
def sort_cont(char_contours):
    """
    To sort contours
    """
    i = 0
    boundingBoxes = [cv2.boundingRect(c) for c in char_contours]
    (char_contours, boundingBoxes) = zip(*sorted(zip(char_contours,
                                                          boundingBoxes),
                                                      key = lambda b: b[1][i],
                                                      reverse = False))
    return char_contours

def segment_chars(plate_image, fixed_width):
    """
    ...
    thresh = cv2.adaptiveThreshold(255, 255,
    ...
    # a canoncial size
    plate_img = imutils.resize(plate_image, width = fixed_width)
    ...
    def check_plate(self, input_img, contour):
        ...
        if self.validateRatio(min_rect):
            ...
            if plateFound:
                ...
                if (characters_on_plate is not None and len(characters_on_plate) <= 20):
